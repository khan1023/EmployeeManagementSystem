...
namespace EMS.Application.User.Handler
{
    public class CreateUserHandler : IRequestHandler<CreateUserCommand, UserResponse>
    {
        ...
        public CreateUserHandler(IUserCommandRepository userCommandRepository, IHostingEnvironment environment, IUserQueryRepository userQueryRepository)
        {
        ...
        public async Task<UserResponse> Handle(CreateUserCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (await _userQueryRepository.CheckDuplicateAsync(request.Name, request.Email) == 0)
                {
                    var userObj = UserMapper.Mapper.Map<UserDetails>(request);

                    string docpath = Path.Combine(_environment.WebRootPath, "Uploads/Documents");
                    if (!Directory.Exists(docpath))
                    {
                        Directory.CreateDirectory(docpath);
                    }
                    string imagepath = Path.Combine(_environment.WebRootPath, "Uploads/UserImages");
                    ...
                    var userStatus = await _userCommandRepository.AddAsync(userObj);
                    if (userStatus.DataId > 0)
                    {
                        response.Message = "User Details has been Created Successfully";
                    }
                    else
                    {
                        throw new ApiException(StatusCodes.Status400BadRequest, "User Details Not Added.");
                    }    
                }
                else
                {
            ...
            {
                throw new ApiException(StatusCodes.Status500InternalServerError, "Error while Adding User Details");
            }
        }
    }
}
